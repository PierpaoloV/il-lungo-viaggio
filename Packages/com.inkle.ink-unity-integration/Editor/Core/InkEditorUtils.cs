...
namespace Ink.UnityIntegration {
	...
	public static class InkEditorUtils {
		// NOTE (porting Il Lungo Viaggio): la classe CreateInkAssetAction derivava
		// da EndNameEditAction, resa obsoleta-errore in Unity 6.5. Rimossa; la
		// creazione di nuovi file .ink dal menu ora avviene direttamente
		// (vedi CreateNewInkFileAtPathWithTemplateAndStartNameEditing).
		...
		public static void CreateNewInkFileAtPathWithTemplateAndStartNameEditing (string filePath, string templateFileLocation) {
			...
			var text = File.Exists(templateFileLocation) ? File.ReadAllText(templateFileLocation) : "";
			var asset = CreateNewInkFileAtPath(filePath, text);
